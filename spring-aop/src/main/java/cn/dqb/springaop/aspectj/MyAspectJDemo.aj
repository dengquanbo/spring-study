...
public aspect MyAspectJDemo {
     ...
     * 定义切点，日志
     */
    pointcut recordLog(): call( * cn.dqb.springaop.service.impl.UserServiceImpl.getUsername(..));

    // 定义切点，权限
    pointcut authCheck(): call( * cn.dqb.springaop.service.impl.*.*(..));

    // 定义切点，返回值
    pointcut getResult(): call( * cn.dqb.springaop.service.impl.*.*(..));

    // 定义切点，异常返回
    pointcut getException(): call( * cn.dqb.springaop.service.impl.*.*(..));

    // 定义切点，环绕通知
    pointcut aroundAdvice(): call( * cn.dqb.springaop.service.impl.*.*(..));
